...
def main():
    ...
    events_result = service.events().list(calendarId='ejohnson98396@gmail.com',
                                          timeMin=now, maxResults=100, singleEvents=True, orderBy='startTime').execute()
    ...
    print('Starting GoogleCalendar crawler; ', datetime.now())
    ...
    for event in events:
        try:
            ...
            if event['start'].get('date') is not None:
                try:
                    table.put_item(Item={"event_id": str(uuid.uuid5(uuid.NAMESPACE_DNS, event['summary'] + event['start'].get('date'))), 
                                        "event_name": event['summary'], 
                                        'description': event['description'], 
                                        'contact_email': "info@seattleadaptivesports.org", 
                                        'event_status': event['status'], 
                                        'location_address': event['location'], 
                                        'event_link': event['htmlLink'], 
                                        'start_date_time': event['start'].get('date'), 
                                        'end_date_time': 'Contact Organizer for Details', 
                                        'activity_type': 'Contact Organizer for Details', 
                                        'org_name': 'Seattle Adaptive Sports', 
                                        'frequency': 'Once', 
                                        'cost': 'Contact Organizer for Details',
                                        'min_age': '?', 
                                        'max_age': '?', 
                                        'disability_types': 'Contact Organizer for Details', 
                                        'inclusive_event': 'Contact Organizer for Details', 
                                        'approver': 'Contact Organizer for Details', 
                    ...
                    print("Event "+ event['summary'] +" exists already.")
            else:
                try:
                    table.put_item(Item={"event_id": str(uuid.uuid5(uuid.NAMESPACE_DNS, event['summary'] + event['start'].get('dateTime'))), 
                                        "event_name": event['summary'], 
                                        'description': event['description'], 
                                        'contact_email': "info@seattleadaptivesports.org", 
                                        'event_status': event['status'], 
            ...
            if event['start'].get('date') is not None:
                try:
                    table.put_item(Item={"event_id": str(uuid.uuid5(uuid.NAMESPACE_DNS, event['summary'] + event['start'].get('date'))), 
                                        ...
                                        'contact_email': "info@seattleadaptivesports.org",  
                                        'event_status': event['status'], 
                                        'location_address': 'Contact Organizer for Details', 
                                        'event_link': event['htmlLink'], 
                                        'start_date_time': event['start'].get('date'), 
                                        'end_date_time': 'Contact Organizer for Details', 
                                        'activity_type': 'Contact Organizer for Details', 
    ...
    print("Closing GoogleCalendar crawler; ", datetime.now())
